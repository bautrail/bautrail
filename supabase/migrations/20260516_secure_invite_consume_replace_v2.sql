drop function if exists public.consume_company_invite(text, text);

create function public.consume_company_invite(p_invite_code text, p_requested_role text default null)
returns table(ok boolean, message text, linked_company_id uuid, linked_role text)
language plpgsql
security definer
